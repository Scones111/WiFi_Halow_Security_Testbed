...
/* add code to handle reconnection logic */
void app_wlan_reconnect(void)
{   
    printf("Still Running\n");
    enum mmwlan_sta_state status = mmwlan_get_sta_state();

    printf("STA state: %u\n", status);

    if (status != MMWLAN_STA_CONNECTED)
    {
        printf("STA not connected, attempting to reconnect...\n");
        app_wlan_stop();
